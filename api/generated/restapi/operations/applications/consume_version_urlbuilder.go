...
func (o *ConsumeVersionURL) Build() (*url.URL, error) {
	...
	var _path = "/applications/{applicationId}"

	applicationID := o.ApplicationID
	if applicationID != "" {
		_path = strings.Replace(_path, "{applicationId}", applicationID, -1)
	} else {
		return nil, errors.New("applicationId is required on ConsumeVersionURL")
	}
...
func (o *ConsumeVersionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ConsumeVersionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ConsumeVersionURL")
	}
